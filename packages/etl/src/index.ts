...
export async function watch(every: number) {
  const job = Cron(`*/${every} * * * *`, { protect: true }, async () => {
    ...
    const { lastUpdate } = (await sql<{ lastUpdate: Date }[]>`SELECT "lastUpdate" FROM "meta" LIMIT 1;`)[0];
    const lastGitHubUpdates = await Promise.all(
      [
        ...
        const response = await fetch(`https://api.github.com/repos/kolmafia/kolmafia/commits?page=1&per_page=1&path=${path}`);
        const json = await response.json() as Endpoints["GET /repos/{owner}/{repo}/commits"]["response"]["data"];
